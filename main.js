...
ipcMain.on(FETCH_TEXT_FROM_STORAGE, () => {
  ...
  storage.get("application", function(error, data) {
    if (error) {
      mainWindow.send(HANDLE_FETCH_TEXT_FROM_STORAGE, {
        success: false,
...
ipcMain.on(SAVE_TEXT_IN_STORAGE, (event, arg) => {
  ...
  storage.set("application", { savedText: arg }, function(error) {
    if (error) {
      mainWindow.send(HANDLE_SAVE_TEXT_IN_STORAGE, {
        success: false,
